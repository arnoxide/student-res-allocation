<?php require_once 'authcontroller.php'; 
  ...
  // $link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "HTTPS" : "HTTPS") . "//" . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'];
  // echo $link;
...
$token = $_GET['token'] ?? '';
 ...
 if(isset($_GET['otp-password']))
 {
   ...
   if($pass=="")
   {
     echo "<center> <b>Please Write Something in the field!</b> </center>";
     exit();
   }
   
   $sql="select * from users where otp like '%$pass%'";
                
   $rs= mysqli_query($conn,$sql);
    
   if(mysqli_num_rows($rs)<1)
   {
    // echo '<script type/javascript> alert("Incorrect OTP")</script>';
    $_SESSION['messages'] = 'Incorrect OTP';
    $_SESSION['types'] = 'danger';
    header('Location: otp-verification?token=' . $token);
...
<form action="<?php echo $_SERVER['PHP_SELF'] . '?token=' . urlencode($_GET['token']); ?>" class="sign-in-form" method="get">
...
<p>
Please enter the code in the field below in order to reset the password.
</p>
...
<input type="submit" name="otp-password" value="Verify code"  class="btn btn-purple btn-lg w-100 hover-lift-light mt-4">
